...
export const useGameLogic = () => {
  ...
  const [room, setRoom] = useState({
    ...
    opponents: {},
    host: "",
    settings: { powerUpsEnabled: false, hardMode: false, secondsToEnd: 0 },
  });
    ...
    const newConnection = new HubConnectionBuilder()
      .withUrl("http://localhost:5000/gamehub", {
        accessTokenFactory: () => {
          ...
          return rawToken ? rawToken.replace(/[^a-zA-Z0-9_.-]/g, "") : "";
        },
      })
      ...
      connection
        ...
        .then(() => {
          connection.on("UpdateState", (gameState) => {
            ...
            const nick = gameState.playerNick || gameState.PlayerNick;
            const progress = gameState.progress ?? gameState.Progress;
            const hasError = gameState.hasError ?? gameState.HasError;
            const wpm = gameState.wpm ?? gameState.Wpm;
            
            if (nick === currentLocalPlayer) {
              setPlayer((p) => ({
                ...p,
                progress: progress,
                hasError: hasError,
          ...
          connection.on("UpdatePlayersList", (data) => {
            setRoom((r) => ({
              ...r,
          ...
          connection.on("GameOver", (winnerName) => {
            setGame((g) => ({ ...g, winner: winnerName, status: "finished" }));
          });

          connection.on("PowerUpGranted", (power) =>
            setPlayer((p) => ({ ...p, powerUp: power })),
          );
          ...
          connection.on("SetUpLobby", (settings) => {
            setRoom((r) => ({
              ...
              settings: {
                ...
                hardMode: settings.hardMode ?? settings.HardMode,
                secondsToEnd: settings.secondsToEnd ?? settings.SecondsToEnd,
              },
            }));
          });
          ...
          connection.on("SettingsUpdate", (newSettings) => {
            setRoom((r) => ({
              ...
              settings: {
                powerUpsEnabled:
                  newSettings.powerUpsEnabled ?? newSettings.PowerUpsEnabled,
                hardMode: newSettings.hardMode ?? newSettings.HardMode,
                secondsToEnd:
                  newSettings.secondsToEnd ?? newSettings.SecondsToEnd,
              },
            }));
          });
            ...
            if (targetNick === localStorage.getItem("username")) {
              setPlayer((p) => {
                ...
                if (power === "bomb") {
                  ...
                  return {
                    ...p,
                    input: p.input.substring(0, cutLength),
                    debuff: "bomb",
                  };
                } else {
              ...
              if (power === "bomb") {
                ...
                setTimeout(
                  () =>
                    setPlayer((p) =>
                      p.debuff === power ? { ...p, debuff: null } : p,
                    ),
                  3000,
                );
              }
            ...
            setRoom((r) => ({
              ...
              chat: [
                ...r.chat,
                {
                  ...
                  sender: senderNick,
                  type:
                    senderNick === localStorage.getItem("username")
                      ? "sent"
                      : "received",
                },
              ],
  ...
  useEffect(() => {
    const localFinished = player.progress === 100;
    const opponentFinished = Object.values(room.opponents).some(opp => opp.progress === 100);
    if ((localFinished || opponentFinished) && game.status === "racing" && game.timeRemaining === null && room.settings.secondsToEnd > 0) {
  ...
  useEffect(() => {
    if (game.status === 'racing' && game.timeRemaining !== null && game.timeRemaining > 0) {
        const timer = setTimeout(() => {
            setGame(g => ({ ...g, timeRemaining: g.timeRemaining - 1 }));
        }, 1000);
        return () => clearTimeout(timer);
    }
  ...
  const handleInputChange = async (e) => {
    ...
    if (connection?.state === "Connected") {
      try {
        await connection.invoke("SendProgress", text);
      } catch (e) {
  ...
  const sendChatMessage = async (messageText) => {
    if (connection?.state === "Connected" && room.code)
  ...
  const handleStart = async () => {
    if (connection?.state === "Connected" && room.code)
      await connection.invoke("StartRoomGame", room.code);
  };
  ...
  const accuracy =
    player.totalKeys > 0
      ? Math.round(
          ((player.totalKeys - player.wrongKeys) / player.totalKeys) * 100,
        )
      : 67;
